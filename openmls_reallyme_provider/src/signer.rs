...
pub struct ReallyMeSuiteSigner {
    ...
    // Public verification keys can identify a member. Erase this owned copy
    // with the signer, consistently with the Ed25519-only signer above.
    public_key: Vec<u8>,
}
...
mod tests {
    ...
    #[test]
    fn suite_signer_erases_owned_key_material() -> Result<(), CryptoError> {
        let mut signer = ReallyMeSuiteSigner::generate(SignatureScheme::ECDSA_SECP384R1_SHA384)?;
        assert!(!signer.public_key().is_empty());
        assert!(!signer.secret_key.expose_secret().is_empty());
        // ZeroizeOnDrop invokes this same zeroization implementation. Check
        // while the allocation is still owned rather than reading freed memory.
        signer.zeroize();
        assert!(signer.public_key().is_empty());
        assert!(signer.secret_key.expose_secret().is_empty());
        Ok(())
    ...
    fn suite_signer_round_trips_ml_dsa_65() -> Result<(), CryptoError> {
        let signer = ReallyMeSuiteSigner::generate(SignatureScheme::MLDSA65)?;
        let payload = b"reallyme-openmls-mldsa65-signer-test";
        let signature = signer
            .sign(payload)
            .map_err(|_| CryptoError::SigningError)?;
        CryptoProvider.verify_signature(
